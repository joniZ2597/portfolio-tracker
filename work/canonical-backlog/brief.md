# Task brief: canonical BACKLOG.md (root-level product backlog)
...
Introduce a single tracked, root-level `BACKLOG.md` as the canonical product backlog / queue of
intent, and add one pointer to it from `AGENTS.md`'s "Repo orientation" section. Documentation
only — no product, runtime, or QA behavior changes.

## Exact files expected to change (locked, no alternatives)

- `BACKLOG.md` (new, tracked, repository root) — content is exactly the Owner-approved
  canonical backlog content from COWORK, reproduced verbatim below in "Locked BACKLOG.md
  content," including the exact final DONE/HISTORY wording: "the completed EG-series items
  identified in the legacy normalization review."
- `AGENTS.md` (Repo orientation section only) — add exactly:
...
- **No other file changes.** `CLAUDE.md` is not modified. No existing legacy backlog/status
  document (e.g. `PORTFOLIO_ENDGAME_QUEUE.md`, `EG-11_QA_MATRIX.md`, anything under
  `.ai-reports/`) is deleted or edited.

## Locked BACKLOG.md content (verbatim, to be written exactly as-is)

The contents inside the fence, excluding the opening and closing fence lines themselves,
are the exact content to write to BACKLOG.md.
...
- `BACKLOG.md` exists, is tracked (`git ls-files BACKLOG.md` non-empty), and is not covered by
  any `.gitignore` rule.
- The `AGENTS.md` pointer text matches exactly what's specified above, added only to "Repo
  orientation," with no other wording in that file changed and no change to workflow semantics
  (LAND/SHIP boundaries, approval points, or the `work/<id>/` convention are untouched).
- `CLAUDE.md` unchanged.
- No legacy backlog/status document deleted or edited.
- Before the implementation commit, the only repository changes are `BACKLOG.md` (new) and
  `AGENTS.md` (modified).
- After the implementation commit, the working tree is clean.
- No product/runtime/QA files touched; `npm run qa:offline` unaffected (docs-only change).
