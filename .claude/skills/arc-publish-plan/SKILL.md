...
```
 ...
 5. Parse source -> PROPOSED plan.json; print it in full;
    require the owner to type CONFIRM
 6. Run P-V1 ... P-V13 + P-V15; any failure -> REFUSE, nothing written
 7. Scan for live claims against the outgoing plan                  [P-V13]
 8. Write plans/.staging-<planId>/ complete
...
Stop and report without publishing if: the runtime root is missing; `AUTHORITY:published-plan`
is already held; the source cannot be parsed into a complete projection; any P-V rule fails;
the owner does not type `CONFIRM`; `plans/<planId>` already exists; a staging directory from
an earlier interrupted run is present; or the request asks this skill to modify a claim, a
mutex holder, `authorized.json`, or the source artifact.
