...
**R-A1 Failed-revision archival.** A revision MAY be archived only when all four hold: `status ∈ {DRAFT, REVIEWED}`; at least one `reviews[]` entry has verdict `FAIL`; the revision has never been `PROMOTED` or `PUBLISHED`; and the owner authorizes the act naming the revision. The act is atomic and does exactly this: the source file is moved **byte-identically** into `.ai-reports/handoffs/archive/` keeping its filename; `revisions[N].source` is set to the new path; `revisions[N].status` is set to `WITHDRAWN`; `revisions[N].archived` is written. `rev`, `sourceHash` and `reviews[]` are unchanged, and `sha256(new source)` MUST still equal `sourceHash` — a byte change REFUSES the act. **`revisions[N].source` may change only through this act**; editing the bytes of any registered revision source remains forbidden in all cases. **No `history[]` entry is written** — no state transition occurs; the record lives in `revisions[N].archived` plus the dated owner-ruling artifact it names. A promoted or published revision can never use R-A1.

The path and the metadata are a **biconditional**, enforced in both directions by `arc.schema.json`: a `source` under `.ai-reports/handoffs/archive/` requires `archived` and `status: WITHDRAWN`, **and** `archived` may exist only on a revision whose `source` is under that prefix. One direction alone would admit archival metadata pinned to a source still sitting in the live corpus — a record claiming an act that never happened. Executable reference: `qa/arc_registry_offline.js` cases `RA1-1`…`RA1-12`.
